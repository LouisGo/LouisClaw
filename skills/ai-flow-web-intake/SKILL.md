...
This is still an intake skill.
It specializes in web-shaped inputs, but it should not automatically turn capture into a long synthesis flow.
...
- Prefer capture first, not immediate analysis.
- Keep stored content clean: URL, title, short summary, or why-it-matters note.
- Unless the user explicitly asks for downstream work, stop after capture.

After capture:

1. Confirm the captured route or resulting record.
2. Stop by default.
3. If the user explicitly wants downstream work, prefer `npm run run`.
4. If they are debugging the intake boundary, prefer `npm run task -- run pull_markdown_sources` or `npm run task -- run process_inbox` explicitly.
...
- Do not dump raw HTML or noisy page chrome into the main pipeline unless the user explicitly wants raw capture.
